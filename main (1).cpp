...
pair<vector<string>, int> mode(vector<string> names, vector<long> grades){
    ...
    for(int i=0; i<grades.size(); i++){
        if(grades[i] == freq[key])
            studentsWithMode.push_back(names[i]);//we willget the mode 
    }
...
int main()
{
    ...
    for(int i=0; i<size; i++){
        cin>>name>>grade;
        names.push_back(name);//enter the names
        grades.push_back(grade);//enter the grade 
    }
    ...
    for(int i=0; p.first.size(); i++){
        cout<<p.first[i]<<" ";//here we will  get the mean and meadian
    }
